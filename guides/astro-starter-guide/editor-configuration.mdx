---
_schema: default
_uuid: b24b5e3f-a48a-4e9b-a602-f65e90b3f5b4
_created_at: '2023-02-01 02:40:30 +0000'
title: Editor configuration
nav_title: Editor configuration
order: 5
published: true
image: >-
  https://cc-dam.imgix.net/documentation/images/getting-started/Guide-Thumbnails/CloudCannon-Getting-Started-Thumbnail-Astro.png
description: >-
  Configure the CloudCannon file editors to give your content team the best
  experience.
tags: []
related_links: []
explicit_canonical: /documentation/guides/universal-starter-guide/editor-configuration/
---
<comp.CommonContent data={{ _file: "/usr/local/__site/src/_common_content/starter-guides/starter-guide-step-5.mdx" }}>
  <comp.SlotContent name="SSG_Visual_Previews_Paragraph">
    ## Visual Editor Previews

    In a previous section of this guide, we talked about the Visual Editor — an intuitive way to edit your site by clicking directly on a preview of your page. But how do you make each region editable in the Visual Editor?

    ### Bookshop

    <a target="_blank" rel="noopener noreferrer" href="https://github.com/CloudCannon/bookshop/">Bookshop</a> is our open-source tool to automate maintaining page components for page building. Using Bookshop, your team can update the front matter of each page component in the Visual Editor. CloudCannon will re-render your page as changes occur, generating a preview of your page as you edit.

    To get started with Bookshop, read our [Astro Bookshop guide](/documentation/guides/bookshop-astro-guide), or [reach out to our support team](/documentation/support).
  </comp.SlotContent>
</comp.CommonContent>